...
def load_checkpoint(load_path, model):
    ...
    model.load-state_dict(state_dict['model_state_dict'])
...
def train(model, optimizer, criterion=nn.BCELoss(), num_epochs=5, eval_every=len(train_loader)//2, best_valid_loss=float("Inf")):
    ...
    for epoch in range(num_epochs):
        for text, label in train_loader:
            optimizer.zero_grad()
            encoded_list = [tokenizer.encode(t, add_specialtokens=True) for t in text]
            padded_list = [e + [0] * (512-len(e)) for e in encoded ist]
